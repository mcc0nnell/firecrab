...
async fn host_local_catalog_row_reinstalls_custom_alias() {
    ...
    assert_eq!(finished.status, ImageInstallStatus::Succeeded, "{}", finished.log);
    assert!(finished.log.contains("local MicroRegistry package checksum verified"));
    let installed = state
        .templates
        .resolve_alias(ALIAS)
...
async fn host_local_catalog_install_rejects_checksum_tampering() {
    ...
    assert!(finished.log.contains("checksum mismatch"), "{}", finished.log);
    assert!(state.templates.resolve_alias(ALIAS).is_none());
}
